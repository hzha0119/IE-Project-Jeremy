    ...
    snapshot.forEach(function (childSnapshot) {
      ...
      if ((postcode == undefined) || (postcode.length == 0)) {
        ...
        if (filterData.length == 0) {
          highScoresList.innerHTML = `<span style="font-size:40px">Sorry</span><br><br>` + 
          `<span style="font-size:20px">We don't have records at that place.</span>`;
        }
        else {
...
function postcodeFunction() {
  ...
  if ((val.length == 0)||((val % 1 == 0) && (val >= 1000) && (val < 10000))){
    ...
    highScoresList.innerHTML = `<span style="font-size:40px">Opps!!!</span><br><br>` + 
    `<span style="font-size:20px">Enter valid postcode.</span>`;
  }
